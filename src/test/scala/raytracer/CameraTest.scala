...
class CameraTest extends FunSuite {
  ...
  test("Camera.test_creation1") {
    ...
    assert(
      c.hsize === 160 && c.vsize === 120 && c.fov === math.Pi / 2 && c.transform === Matrix
        .getIdentityMatrix(4))
  }
  ...
  test("Camera.test_raypixel_3") {
    val c: Camera =
      Camera(201, 101, math.Pi / 2).setTransform(RotationY(math.Pi / 4) * Translation(0, -2, 5))
    val r: Ray = c.rayForPixel(100, 50)

    assert(
      r.origin === Point(0, 2, -5) && r.direction === Vector(math.sqrt(2) / 2,
                                                             0,
                                                             -math.sqrt(2) / 2))
  }
  test("Camera.test_render") {
    val w: World = World.defaultWorld
    val c: Camera = Camera(11, 11, math.Pi / 2)
      .setTransform(viewTransform(Point(0, 0, -5), Point(0, 0, 0), Vector(0, 1, 0)))
