...
class IntersectionTest extends FunSuite {
  ...
  test("intersection.test_glass_sphere0") {
    val a: Sphere = Sphere
      ...
      .setMaterial(Sphere.glassSphere().material.setRefractiveIndex(1.5))
    val b: Sphere = Sphere
      .glassSphere()
      .setTransform(Translation(0, 0, -0.25))
      .setMaterial(Sphere.glassSphere().material.setRefractiveIndex(2.0))
    val c: Sphere = Sphere
      .glassSphere()
      .setTransform(Translation(0, 0, 0.25))
      .setMaterial(Sphere.glassSphere().material.setRefractiveIndex(2.5))
    val r: Ray = Ray(Point(0, 0, -4), Vector(0, 0, 1))
    val xs: Seq[Intersection] = Intersection.intersections(Intersection(2, a),
                                                           Intersection(2.75, b),
                                                           Intersection(3.25, c),
                                                           Intersection(4.75, b),
                                                           Intersection(5.25, c),
  ...
  test("intersection.test_schlick_total_internal_reflection") {
    ...
    val xs: Seq[Intersection] = Intersection.intersections(Intersection(-math.sqrt(2) / 2, a),
                                                           Intersection(math.sqrt(2) / 2, a))
    val comps: Computation = Computation.prepareComputations(xs(1), r, xs)
