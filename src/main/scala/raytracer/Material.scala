...
class Material(val colour: Colour,
               val ambient: Double,
               val diffuse: Double,
               val specular: Double,
    ...
    that match {
      case that: Material =>
        (colour === that.colour) && doubleEq(ambient, that.ambient) &&
          doubleEq(diffuse, that.diffuse) && doubleEq(specular, that.specular) && doubleEq(
          shininess,
          that.shininess)
      case _ => false
    }
  ...
  final def ===(that: Material): Boolean = {
    (colour === that.colour) && doubleEq(ambient, that.ambient) &&
    doubleEq(diffuse, that.diffuse) && doubleEq(specular, that.specular) && doubleEq(shininess,
                                                                                     that.shininess)
  }

  final override def hashCode: Int = (colour, ambient, diffuse, specular, shininess).##

  def setColour(c: Colour): Material =
    new Material(c,
                 ambient,
                 diffuse,
                 specular,
  ...
  def setAmbient(x: Double): Material =
    new Material(colour,
                 x,
                 diffuse,
                 specular,
  ...
  def setDiffuse(x: Double): Material =
    new Material(colour,
                 ambient,
                 x,
                 specular,
  ...
  def setSpecular(x: Double): Material =
    new Material(colour,
                 ambient,
                 diffuse,
                 x,
  ...
  def setShininess(x: Double): Material =
    new Material(colour,
                 ambient,
                 diffuse,
                 specular,
  ...
  def setPattern(x: Pattern): Material =
    new Material(colour,
                 ambient,
                 diffuse,
                 specular,
  ...
  def setReflective(x: Double): Material =
    new Material(colour,
                 ambient,
                 diffuse,
                 specular,
                 ...
                 transparency,
                 refractiveIndex)
  def setTransparency(x: Double): Material =
    new Material(colour,
                 ambient,
                 diffuse,
                 specular,
                 ...
                 x,
                 refractiveIndex)
  def setRefractiveIndex(x: Double): Material =
    new Material(colour,
                 ambient,
                 diffuse,
                 specular,
  ...
  def specularContribution(light_dot_normal: Double,
                           reflect_dot_eye: Double,
                           light_intensity: Colour): Colour = {
    if (light_dot_normal < 0 || reflect_dot_eye <= 0) Colour.black
    else light_intensity * specular * math.pow(reflect_dot_eye, shininess)
  }
  ...
  def lighting(obj: SpaceObject,
               light: Light,
               p: RTTuple,
               eyev: RTTuple,
               normalv: RTTuple,
               in_shadow: Boolean): Colour = {
    ...
    if (in_shadow)(ambientContribution(effective_colour))
    else
